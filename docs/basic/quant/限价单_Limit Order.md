...
限价单是构成交易所订单簿的基本元素。订单簿是一个按价格排列的买卖限价单集合，反映了市场在不同价位上的供需关系。
...
此时：
- **最优卖价（Ask）**= 10.01元，**最优买价（Bid）**= 10.00元
- **买卖价差（Spread）**= 10.01 - 10.00 = 0.01元
- 如果你下达一个10.00元的买入限价单（500股），它会排在当前800股买入单之后，等待10.00元的卖单出现时按照时间优先原则成交

### 成交概率与限价单定价

限价单的成交概率取决于限价与当前市场价格的距离。在学术文献中，限价单的成交概率可以近似为：

$$ P(\text{成交}) \approx e^{-\lambda \cdot |p_{limit} - p_{mid}|} $$

其中 $\lambda$ 为衰减参数（取决于流动性），$p_{limit}$ 为限价，$p_{mid}$ 为买卖中间价。限价越远离当前价格，成交概率越低，但一旦成交则获得更优的价格改善。

## 量化应用：算法交易中的限价单策略

在量化交易中，限价单的使用远比简单的手动下单复杂。算法交易系统通过精密的数学模型来优化限价单的放置和管理。

### TWAP与VWAP算法

两种最常见的限价单执行算法：

**时间加权平均价格（TWAP）**：将大额订单均匀分割在指定时间段内，以限价单方式逐步执行。例如，要在2小时内买入10万股，每12分钟提交一批1万股的限价单。

**成交量加权平均价格（VWAP）**：根据历史日内成交量分布来分配订单数量。在成交量高峰时段（如开盘和收盘前）提交更多的限价单，在低成交量时段提交较少的单量。

$$ VWAP = \frac{\sum_{i=1}^{N} P_i \times V_i}{\sum_{i=1}^{N} V_i} $$

其中 $P_i$ 和 $V_i$ 分别为第 $i$ 笔成交的价格和数量。

### 做市策略中的限价单

做市商（Market Maker）通过同时在买方和卖方放置限价单来提供流动性并赚取买卖价差。典型的做市策略会在中间价两侧对称放置限价单：

- 买入限价 = $p_{mid} - \frac{s}{2}$
- 卖出限价 = $p_{mid} + \frac{s}{2}$

其中 $s$ 为目标价差。做市商的收益来源于价差，但面临逆向选择风险（即知情交易者在价格即将大幅变动时与之成交）。

### 限价单执行质量的评价

交易者通过以下指标评估限价单的执行质量：

- **执行缺口（Implementation Shortfall）**：实际成交价格与决策价格的差异
- **价格改善（Price Improvement）**：相比市价单，限价单获得的价格优势
- **填充率（Fill Rate）**：限价单实际成交数量占下单总量的比例
- **市场冲击（Market Impact）**：订单执行对市场价格造成的不利影响
