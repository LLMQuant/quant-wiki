...
1. **突破交易**：当价格突破关键技术位时，快速执行比价格优化更重要
2. **风险管理**：当需要紧急平仓止损时，延迟执行可能造成更大损失
3. **信息驱动交易**：在新闻发布后的窗口期内，速度决定了能否捕获Alpha

## 订单类型对比

| 订单类型 | 执行速度 | 价格控制 | 适用场景 |
|---------|---------|---------|---------|
| **限时订单（Held）** | 即时 | 无 | 紧急执行、止损 |
| 非限时订单（Not-Held） | 灵活 | 经纪人裁量 | 大额订单、低波动 |
| 限价单（Limit） | 不确定 | 完全控制 | 价格敏感 |
| IOC订单 | 即时（部分成交） | 限价 | 流动性探测 |

## 量化交易中的执行优化

在算法交易领域，限时订单的概念与**激进执行（Aggressive Execution）**策略密切相关：

$$
\text{执行成本} = \text{市场冲击} + \text{时机成本} + \text{价差成本}
$$

- **市场冲击**：大额限时订单可能推动价格不利变动
- **时机成本**：延迟执行面临的价格漂移风险
- **价差成本**：以对手方最优价格成交的固有成本

TWAP和VWAP等算法通过将大额订单拆分为小单，在速度和成本之间寻求平衡。
