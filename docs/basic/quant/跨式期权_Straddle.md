...
长跨式期权的总成本为看涨期权和看跌期权的权利金之和：

$$ Cost_{straddle} = C(S, K, T, \sigma) + P(S, K, T, \sigma) $$

其中 $C$ 和 $P$ 分别为看涨和看跌期权的Black-Scholes价格。根据看涨-看跌平价关系（Put-Call Parity）：

$$ C - P = S - K \cdot e^{-rT} $$

当行使价格 $K = S \cdot e^{rT}$（即远期平值）时，$C = P$，跨式期权的成本为：

$$ Cost_{straddle} = 2C \approx 2 \cdot S \cdot N\left(\frac{\sigma\sqrt{T}}{2}\right) - S \approx S \cdot \sigma \cdot \sqrt{\frac{2T}{\pi}} $$

这一近似表明，平值跨式期权的成本大约与标的价格、隐含波动率和到期时间的平方根成正比。

### 盈亏分析

长跨式期权到期时的盈亏函数为：

$$ P\&L = \max(S_T - K, 0) + \max(K - S_T, 0) - Cost_{straddle} = |S_T - K| - Cost_{straddle} $$

盈亏平衡点为：

$$ S_{BE}^{upper} = K + Cost_{straddle} $$
$$ S_{BE}^{lower} = K - Cost_{straddle} $$

### 跨式期权的希腊值

| 希腊值 | 长跨式期权 | 短跨式期权 |
|-------|-----------|-----------|
| Delta | 接近零（平值时） | 接近零（平值时） |
| Gamma | 正值（较大） | 负值（较大） |
| Theta | 负值（时间衰减） | 正值（时间收益） |
| Vega  | 正值（受益于波动率上升） | 负值（受益于波动率下降） |
...
交易者将一份3月15日的55美元看涨期权和一份3月15日的55美元看跌期权的价格相加，以确定创建跨式期权的成本。如果这两份合约的交易价格均为2.50美元，那么支付的权利金总额为5.00美元 x 100 = 500美元（每份合约包含100股）。
...
def straddle_analysis(S, K, T, r, sigma):
    """
    跨式期权综合分析
    ...
    dict -- 跨式期权分析结果
    """
    call_price = bs_option_price(S, K, T, r, sigma, 'call')
    put_price = bs_option_price(S, K, T, r, sigma, 'put')
    
    straddle_cost = call_price + put_price
    
    # 盈亏平衡点
    upper_be = K + straddle_cost
    lower_be = K - straddle_cost
    
    # 隐含波动区间（标的需要移动的百分比）
    implied_move = straddle_cost / S
    
    # 希腊值
    d1 = (np.log(S / K) + (r + 0.5 * sigma**2) * T) / (sigma * np.sqrt(T))
    
    delta = norm.cdf(d1) + (norm.cdf(d1) - 1)  # call_delta + put_delta
    gamma = 2 * norm.pdf(d1) / (S * sigma * np.sqrt(T))
    vega = 2 * S * norm.pdf(d1) * np.sqrt(T) / 100
    theta = 2 * (-(S * norm.pdf(d1) * sigma) / (2 * np.sqrt(T))) / 365
    ...
    return {
        ...
        'implied_move_pct': f"{implied_move:.2%}",
        'delta': round(delta, 4),
        'gamma': round(gamma, 6),
        'vega': round(vega, 4),
        'daily_theta': round(theta, 4)
    }


def straddle_pnl_at_expiry(K, cost, price_range):
    """计算到期日跨式期权的盈亏"""
    return np.abs(price_range - K) - cost


# 使用示例
result = straddle_analysis(S=100, K=100, T=30/365, r=0.05, sigma=0.30)
print("跨式期权分析结果:")
...
跨式期权是波动率交易的核心工具。其核心逻辑在于：

**买入跨式期权 = 做多波动率**

当交易者认为隐含波动率（IV）被低估，即未来的已实现波动率（RV）将高于当前隐含波动率时，可以买入跨式期权。盈利条件为：

$$ RV > IV \implies P\&L > 0 $$

反之，**卖出跨式期权 = 做空波动率**，适用于隐含波动率被高估的场景。

### 波动率微笑与跨式期权定价

在实际市场中，不同行使价格的期权具有不同的隐含波动率，形成所谓的"波动率微笑"（Volatility Smile）。平值跨式期权的隐含波动率通常是波动率曲面的基准参考点，被称为ATM Vol。跨式期权的市场报价常被直接用于校准波动率模型。

### 财报季跨式期权策略

在量化策略中，财报前的跨式期权交易是一种常见策略。策略逻辑如下：

1. 统计该股票历史财报日的实际波动幅度
2. 将其与当前跨式期权隐含的波动幅度进行比较
3. 如果历史波动幅度显著大于隐含幅度，买入跨式期权
4. 如果历史波动幅度显著小于隐含幅度，卖出跨式期权
...
## 跨式期权与宽跨式期权（Strangle）的比较

宽跨式期权（Strangle）与跨式期权类似，但使用不同的行使价格：

| 特征 | 跨式期权（Straddle） | 宽跨式期权（Strangle） |
|------|-------------------|---------------------|
| 行使价格 | 看涨和看跌相同（ATM） | 看涨OTM，看跌OTM |
| 成本 | 较高 | 较低 |
| 盈亏平衡区间 | 较窄 | 较宽 |
| 最大损失 | 较大（权利金更高） | 较小（权利金更低） |
| Gamma | 更大 | 更小 |
| Vega | 更大 | 更小 |

宽跨式期权的盈亏函数为：

$$ P\&L = \max(S_T - K_{call}, 0) + \max(K_{put} - S_T, 0) - Cost_{strangle} $$
...
在量化交易实践中，跨式期权是波动率交易的基础工具。通过比较隐含波动率与预期已实现波动率，量化交易者可以系统性地识别跨式期权的错误定价机会。结合历史波动率分析、事件驱动研究和希腊值风险管理，跨式期权策略可以成为量化投资组合中重要的收益来源。
## 关于LLMQuant
