...
假设某投资组合经理希望评估其基金相对于Fama-French三因子模型的表现。已知以下数据（年化）：
...
通过回归分析得到因子载荷：$\beta_1 = 1.1$，$\beta_2 = 0.45$，$\beta_3 = 0.30$

则模型预测的超额收益为：

$$ R_i - R_f = \alpha + 1.1 \times 7\% + 0.45 \times 3.2\% + 0.30 \times 4.8\% $$

$$ = \alpha + 7.7\% + 1.44\% + 1.44\% = \alpha + 10.58\% $$

实际超额收益为 $12\% - 3\% = 9\%$，因此：

$$ \alpha = 9\% - 10.58\% = -1.58\% $$

负的alpha值表明，该投资组合经理在承受了较高的规模风险和价值风险敞口后，实际未能创造超额收益，反而有 -1.58% 的负alpha。这说明该组合的收益完全可以通过被动暴露于三个因子来解释，经理并未展现出真正的选股能力。

## 量化应用：因子投资策略构建

Fama-French模型在量化投资实践中有广泛的应用，以下是构建因子投资策略的典型方法：

### 因子组合构建步骤

1. **因子定义与数据获取**：获取全市场股票的市值、账面价值、价格等数据。在A股市场中，通常使用Wind、Tushare或聚宽等数据源。

2. **股票分组排序**：按照市值（Size）和账面市值比（B/M）对股票进行双重排序。典型做法是将股票按市值中位数分为大盘股和小盘股两组，再按B/M的30%和70%分位数分为低、中、高三组，形成2x3共6个投资组合。

3. **计算因子收益**：
   - $SMB = \frac{1}{3}(S/L + S/M + S/H) - \frac{1}{3}(B/L + B/M + B/H)$
   - $HML = \frac{1}{2}(S/H + B/H) - \frac{1}{2}(S/L + B/L)$

4. **定期再平衡**：通常每年6月底按照上一年末的财务数据对投资组合进行重新排序和调整。

### A股市场的经验证据

在中国A股市场中，小盘股效应和价值效应的表现呈现出与美股市场不同的特征：

- **规模效应**：2010-2015年间，A股小盘股显著跑赢大盘股，年化超额收益约为10%-15%。但自2017年以来，随着市场风格切换和机构化进程加速，小盘股效应有所减弱。
- **价值效应**：在A股市场中，高账面市值比的价值股相对成长股的溢价并不稳定，受到市场情绪和政策影响较大。
- **本土化调整**：在应用Fama-French模型于A股时，研究者通常需要进行本土化调整，例如使用流通市值代替总市值、调整财务数据的披露时滞等。
