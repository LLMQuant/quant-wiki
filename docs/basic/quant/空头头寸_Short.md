...
$$ \text{利润} = (P_{卖出} - P_{买回}) \times Q - C_{借券} - C_{交易} $$

其中 $P_{卖出}$ 为卖出（开仓）价格，$P_{买回}$ 为买回（平仓）价格，$Q$ 为股票数量，$C_{借券}$ 为借券费用，$C_{交易}$ 为交易佣金和费用。

### 详细计算示例

假设交易者以每股200元做空1000股某科技公司股票：

- 卖出收入 = 200 x 1000 = 200,000元
- 需要缴纳的初始保证金（假设50%）= 100,000元
- 年化借券费率 = 2%
- 持仓30天后，股价跌至170元，交易者平仓

利润计算：
- 价差收益 = (200 - 170) x 1000 = 30,000元
- 借券费用 = 200,000 x 2% x (30/365) = 328.77元
- 交易佣金（假设双边万分之三）= (200,000 + 170,000) x 0.03% = 111元
- 净利润 = 30,000 - 328.77 - 111 = 29,560.23元
- 投入资本回报率 = 29,560.23 / 100,000 = 29.56%（30天内）

### 做空成本的构成

| 成本类别 | 说明 | 典型范围 |
|----------|------|----------|
| 借券费用 | 向券商支付的股票借入费用 | 年化0.3%-30%不等 |
| 保证金利息 | 保证金账户中的利息支出 | 与券商利率相关 |
| 股息补偿 | 做空期间若派发股息，需向出借方补偿 | 等于实际股息金额 |
| 交易佣金 | 买卖交易的手续费 | 万分之二至千分之一 |
| 印花税 | 部分市场卖出时征收 | 如A股卖出千分之一 |

## 量化应用：做空策略与多空组合

### 多空策略（Long-Short Strategy）

量化对冲基金最常见的策略之一就是多空股票策略（Equity Long-Short），其核心思路是：

1. **做多**被低估或具有正alpha的股票
2. **做空**被高估或具有负alpha的股票
3. 通过对冲消除市场系统性风险（Beta），仅保留alpha收益

组合的预期收益为：

$$ R_{portfolio} = \alpha_{long} + \alpha_{short} + \beta_{net} \times R_{market} $$

在市场中性策略中，$\beta_{net} \approx 0$，因此组合收益主要来自多空两端的alpha。

### 做空信号的典型因子

在量化模型中，以下因子常被用于识别做空机会：

- **盈利质量因子**：应计利润占比高、盈利质量低的公司
- **估值因子**：市盈率、市净率极高的股票
- **动量反转因子**：短期过度上涨后可能回调的股票
- **内部人交易**：管理层大量减持的股票
- **融资融券数据**：融券余额持续上升的标的
...
如果被做空证券的价格开始上涨而非下跌，损失会迅速增加。实际上，由于证券价格没有上限，空头头寸的损失在理论上是无限的。鉴于这种固有风险和交易的复杂性，做空证券通常仅建议给予更先进的交易者和投资者。在量化交易领域，做空是构建市场中性策略和对冲系统性风险的核心工具，但需要精密的风险管理系统来控制尾部风险。
