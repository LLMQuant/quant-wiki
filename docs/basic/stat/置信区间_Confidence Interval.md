...
当总体标准差 $\sigma$ 已知时，总体均值 $\mu$ 的 $100(1-\alpha)\%$ 置信区间为：
...
当总体标准差未知，用样本标准差 $s$ 代替时：

$$
\bar{X} \pm t_{\alpha/2, n-1} \cdot \frac{s}{\sqrt{n}}
$$

其中 $t_{\alpha/2, n-1}$ 为自由度 $n-1$ 的 t 分布的上 $\alpha/2$ 分位数。
...
置信区间的半宽度（误差界限）为 $E = z_{\alpha/2} \cdot \sigma / \sqrt{n}$，由此可得所需最小样本量：
...
假设某量化基金过去 36 个月的月度收益率数据如下：样本均值 $\bar{X} = 1.2\%$，样本标准差 $s = 3.5\%$。

**95% 置信区间（使用 t 分布）：**

$$
1.2\% \pm t_{0.025, 35} \times \frac{3.5\%}{\sqrt{36}} = 1.2\% \pm 2.030 \times 0.583\% = 1.2\% \pm 1.184\%
$$

因此 95% 置信区间为 $[0.016\%, 2.384\%]$。

由于该区间不包含零，我们可以在 95% 置信水平下认为该基金的月均收益率显著大于零。

**99% 置信区间：**

$$
1.2\% \pm t_{0.005, 35} \times 0.583\% = 1.2\% \pm 2.724 \times 0.583\% = 1.2\% \pm 1.588\%
$$

99% 置信区间为 $[-0.388\%, 2.788\%]$，此区间包含零，说明在更严格的置信水平下无法排除收益率为零的可能性。

## 金融应用

置信区间在量化金融中有着广泛且关键的应用：

**1. 夏普比率的置信区间：** 夏普比率 $\hat{SR}$ 的近似标准误差为：
...
$$
\hat{SR} \pm 1.96 \times SE(\hat{SR})
$$

**2. VaR 估计的不确定性：** 历史模拟法计算的 VaR 本身是一个估计量，其置信区间可以通过 Bootstrap 方法获得。

**3. 回归系数的置信区间：** 在因子模型中，回归系数 $\hat{\beta}$ 的置信区间为：

$$
\hat{\beta} \pm t_{\alpha/2, n-k-1} \times SE(\hat{\beta})
$$

若置信区间包含零，则该因子在统计上不显著。

**4. 蒙特卡洛模拟的精度：** 期权蒙特卡洛定价的置信区间由中心极限定理保证：

$$
\hat{C} \pm z_{\alpha/2} \times \frac{s}{\sqrt{N}}
$$
...
# 计算策略月度收益率的置信区间
monthly_returns = np.array([0.015, -0.008, 0.022, 0.005, 0.018, -0.003,
                            0.012, 0.009, 0.016, -0.005, 0.020, 0.007,
                            0.014, 0.003, 0.019, -0.002, 0.011, 0.008,
                            0.017, 0.001, 0.013, 0.006, 0.021, -0.004,
                            0.010, 0.015, 0.004, 0.018, -0.001, 0.012,
                            0.016, 0.007, 0.014, 0.002, 0.019, 0.009])

n = len(monthly_returns)
mean = np.mean(monthly_returns)
se = stats.sem(monthly_returns)  # 标准误差

# 不同置信水平的区间
for confidence in [0.90, 0.95, 0.99]:
    ci = stats.t.interval(confidence, df=n-1, loc=mean, scale=se)
    print(f"{confidence:.0%} 置信区间: [{ci[0]:.4%}, {ci[1]:.4%}]")

# 夏普比率的置信区间
sharpe = (mean / np.std(monthly_returns, ddof=1)) * np.sqrt(12)
se_sharpe = np.sqrt((1 + 0.5 * sharpe**2) / n) * np.sqrt(12)
print(f"\n年化夏普比率: {sharpe:.2f}")
print(f"夏普比率 95% CI: [{sharpe - 1.96*se_sharpe:.2f}, {sharpe + 1.96*se_sharpe:.2f}]")

# Bootstrap 置信区间
np.random.seed(42)
n_bootstrap = 10000
boot_means = [np.mean(np.random.choice(monthly_returns, n, replace=True))
              for _ in range(n_bootstrap)]
ci_boot = np.percentile(boot_means, [2.5, 97.5])
print(f"\nBootstrap 95% 置信区间: [{ci_boot[0]:.4%}, {ci_boot[1]:.4%}]")
```
...
在量化金融中，置信区间不仅用于参数估计，还直接影响投资决策。例如，若某策略年化收益率的 95% 置信区间的下界大于零，则可以更有信心地分配资金。

## 关于置信区间的常见误解是什么？

关于置信区间的最大误解是，它们代表从给定样本中数据落在上下界限之间的百分比。换句话说，假设99%的置信区间意味着99%的随机样本数据落在这些界限之间是错误的。实际上，这意味着可以有99%的信心这个范围将包含总体均值。

更精确地说：如果我们重复抽样并构建置信区间的过程无限次，那么 $100(1-\alpha)\%$ 的区间将包含真实参数值。每个具体的置信区间要么包含真实值，要么不包含——它不是一个概率陈述。
...
p 值与置信区间的关系可以简洁地表述为：**在双尾检验中，如果 $100(1-\alpha)\%$ 置信区间不包含零假设参数值，则 p 值小于 $\alpha$，反之亦然。** 两者提供了等价的信息，但置信区间还额外提供了效应量的估计范围。

## 结论

置信区间使分析师能够理解统计分析结果是真实的还是偶然的。当试图根据样本数据进行推断或预测时，确实会存在一些不确定性，即分析结果是否与被研究的真实总体相符。置信区间描绘了可能的真实值应当落入的范围。

在量化金融实践中，置信区间是评估策略绩效、风险度量和模型参数不确定性的核心工具。相比于仅报告点估计值，置信区间提供了更全面的信息，有助于做出更稳健的投资决策。
