...
为帮助读者建立直觉，下表提供了各类策略在长期历史中的典型表现特征（基于美国市场回测数据，仅供参考，不代表未来表现）：

| 策略 | 年化收益（约） | 年化波动率（约） | 最大回撤（约） | 夏普比率（约） |
|------|:---:|:---:|:---:|:---:|
| 策略一：纯标普500 | 10-11% | 15-16% | -50%+ | 0.6-0.7 |
| 策略二：90/10股债 | 9-10% | 13-14% | -45% | 0.6-0.7 |
| 策略三：三基金(60/20/20) | 8-9% | 12-13% | -40% | 0.5-0.6 |
| 策略六：Bernstein四等分 | 8-9% | 11-12% | -35% | 0.5-0.6 |
| 策略八：咖啡馆组合 | 7-8% | 8-10% | -25% | 0.5-0.7 |
| 策略九：终极买入持有 | 7-9% | 9-11% | -30% | 0.5-0.6 |

> **注意**：以上数据为近似历史范围，实际表现会因具体时间窗口、再平衡频率、费率等因素而有所不同。

### 核心规律

从数据中可以观察到几个规律：

1. **股票占比越高，长期收益越高，但波动和最大回撤也越大**。纯股票策略在牛市中表现最好，但在2008年金融危机中跌幅可超过50%。
2. **加入债券能显著降低波动率和最大回撤**，但代价是长期收益的适度下降。
3. **更多元化的组合（如加入REIT、商品、国际小盘）在某些特定周期中可能表现更好**，但整体夏普比率的提升并不显著——管理复杂度的增加是否值得，需要投资者自行权衡。

## 实施细节：再平衡与税务考量

选择了策略之后，**如何执行**同样重要。以下是几个实操要点：

### 再平衡（Rebalancing）

再平衡是指定期将各资产的实际比例调整回目标配比。常见方法有：

- **日历法**：每季度或每年固定时间再平衡一次。
- **阈值法**：当某一资产的实际占比偏离目标超过一定幅度（如5个百分点）时触发再平衡。

```python
def need_rebalance(current_weights, target_weights, threshold=0.05):
    """
    判断是否需要再平衡

    参数:
        current_weights: dict, 当前各资产权重
        target_weights: dict, 目标各资产权重
...
# 示例
target = {'US_Stock': 0.60, 'Intl_Stock': 0.20, 'Bond': 0.20}
current = {'US_Stock': 0.68, 'Intl_Stock': 0.17, 'Bond': 0.15}
print(need_rebalance(current, target))  # True, 因为US_Stock偏离了8个百分点
```

### 税务效率

在应税账户中，频繁再平衡会触发资本利得税。几个应对策略：

- **利用新资金再平衡**：将新投入的资金分配到权重偏低的资产，而非卖出权重偏高的资产。
- **税损收割（Tax-Loss Harvesting）**：卖出亏损头寸以抵消其他资本利得。
- **资产放置优化**：将税务效率低的资产（如债券、REIT）放在退休账户中，将税务效率高的资产（如指数基金）放在应税账户中。
