...
马可维茨理论的一个重要推论是**两基金分离定理**：有效前沿上的任何组合，都可以表示为前沿上两个不同组合的线性组合。换言之，投资者只需持有两个"基准"基金，就能通过调整两者的比例复制有效前沿上的任意一点。

数学表达如下：若 $$w_A$$ 和 $$w_B$$ 是有效前沿上两个不同的组合权重，则对任意有效组合 $$w^*$$，存在标量 $$\alpha \in \mathbb{R}$$ 使得：

$$ w^* = \alpha w_A + (1 - \alpha) w_B $$

当引入无风险资产后，所有投资者的最优组合都是无风险资产与**切线组合（Tangency Portfolio）** 的混合，这就是著名的**资本市场线（Capital Market Line, CML）**。切线组合的权重为：

$$ w_{\text{tan}} = \frac{\Sigma^{-1}(\mu - R_f \mathbf{1})}{\mathbf{1}^T \Sigma^{-1}(\mu - R_f \mathbf{1})} $$

其中 $$R_f$$ 为无风险利率，$$\mathbf{1}$$ 为全1向量。

### Python实战：计算有效前沿

以下代码演示如何用Python计算并绘制有效前沿：
...
# 假设3只资产的年化预期收益率和协方差矩阵
mu = np.array([0.12, 0.08, 0.15])  # 预期收益率
cov = np.array([
    [0.04,  0.006, 0.010],
    [0.006, 0.02,  0.004],
    [0.010, 0.004, 0.06 ]
...
plt.plot(frontier_vols, frontier_rets, 'b-', linewidth=2, label='有效前沿')
plt.scatter([np.sqrt(cov[i, i]) for i in range(n_assets)], mu,
            color='red', s=80, zorder=5, label='单一资产')
plt.xlabel('波动率 (σ)')
plt.ylabel('期望收益率 (μ)')
...
运行以上代码，你可以直观看到：有效前沿是一条从最小方差组合向右上方延伸的弧线，而单一资产的点通常位于前沿的"内部"——这正说明了分散化能在同等风险下提升收益。

### 实务中的注意事项

在实际量化投资中应用MPT时，有几个关键问题需要注意：

1. **估计误差敏感性**：协方差矩阵 $$\Sigma$$ 和预期收益 $$\mu$$ 的微小估计误差可能导致最优权重的巨大变化。实务中常采用**收缩估计（Shrinkage Estimation）**（如Ledoit-Wolf方法）来改善协方差矩阵的稳定性。

2. **Black-Litterman模型**：为解决预期收益难以准确估计的问题，Black和Litterman提出了将市场均衡收益与投资者主观观点相融合的框架，生成更稳健的预期收益向量。

3. **约束条件**：实际投资中通常需要加入做空限制、单一资产上限、行业集中度限制等约束，使得优化问题变为带约束的二次规划。
